...
function angleBetweenClockHands(date) {
  ...
  let hourAngle = 0.5 * (60 * hour + minute);
  let minuteAngle = 6 * minute;
  if (hourAngle >= 360) hourAngle -= 360;
  if (minuteAngle >= 360) minuteAngle -= 360;
  let angle = Math.max(minuteAngle, hourAngle) - Math.min(minuteAngle, hourAngle);
  // const a = hourAngle - minuteAngle;
  // const angle = Math.min(a, Math.abs(360 - a));
  // console.log(hour, minute, hourAngle, minuteAngle, angle);
